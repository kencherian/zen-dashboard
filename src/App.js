...
function App() {
  ...
  const [weather, setWeather] = useState({ temp: "33", desc: "pretty sunny" });
  const getRandomQuote = () => {
    const randomIndex = Math.floor(Math.random() * myQuotes.length);
    setQuote(myQuotes[randomIndex]);
  };

  // 1. Create memory for 3 specific tasks
  ...
  useEffect(() => {
  const API_KEY = "b53958bda0b5b6f7e247c2f9c437285d"; // Put your actual key inside the quotes
  const city = "Nagpur";
  ...
  // 2. The Logic: This function "flips" a task from done to not done
  const toggleTask = (id) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex flex-col items-center py-20 px-4">
      ...
      <header className="mb-16 text-center">
        <h1 className="text-sm uppercase tracking-[0.3em] text-gray-400 mb-2">Morning, Commander</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
       ...
       {/* THE VIBE */}
<div className="p-8 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between min-h-[250px] relative group">
  <div>
  ...
  <div className="flex justify-between items-end mt-4">
    ...
    >
      New Briefing
        ...
        {/* THE ATMOSPHERE (Now Live!) */}
        <div className="p-8 bg-white rounded-2xl shadow-sm border border-gray-100">
        <h3 className="text-xs uppercase tracking-widest text-gray-400 mb-6">The Atmosphere</h3>
        ...
        {/* THE FOCUS (Now Interactive!) */}
        <div className="p-8 bg-white rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-xs uppercase tracking-widest text-gray-400 mb-6">The Focus</h3>
